...
class OpenTelemetry(OTELGenAISemconvMixin, CustomLogger):
    ...
    def get_tracer_to_use_for_request(self, kwargs: dict) -> Tracer:
        ...
        if dynamic_headers is not None:
            ...
            verbose_logger.debug(
                "[OTEL DEBUG] Using DYNAMIC tracer with headers: %s",
                redact_string(str(dynamic_headers)),
            )
        else:
    ...
    def _get_dynamic_otel_config_from_kwargs(
        self, kwargs: dict
        ...
        return self.construct_dynamic_otel_config(
            standard_callback_dynamic_params=standard_callback_dynamic_params
        )
    ...
    def _get_tracer_with_dynamic_config(self, dynamic_config: OpenTelemetryConfig):
        ...
        temp_provider.add_span_processor(
            self._get_span_processor(config_override=dynamic_config)
        )
    ...
    def _get_span_processor(
        ...
        otel_exporter = (
            config_override.exporter if config_override else self.OTEL_EXPORTER
        )
        otel_endpoint = (
            config_override.endpoint if config_override else self.OTEL_ENDPOINT
        )
        ...
        _split_otel_headers = OpenTelemetry._get_headers_dictionary(
            headers=dynamic_headers or otel_headers
        )
        ...
        if otel_exporter == "console":
            ...
            otel_exporter == "otlp_http"
            or otel_exporter == "http/protobuf"
            or otel_exporter == "http/json"
        ):
            try:
