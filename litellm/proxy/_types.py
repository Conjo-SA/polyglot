...
class ConfigList(LiteLLMPydanticObjectBase):
    ...
    field_options: Optional[list[str]] = (
        None  # Allowed values, for field_type == "Select"
    )
    field_tab: Optional[str] = (
        None  # Admin UI sub-tab this field renders under; None groups it with the rest
    )
...
class CoordinationRedisParams(LiteLLMPydanticObjectBase):
    ...
    url: Optional[str] = Field(
        None, description="full Redis connection url, e.g. redis://:pass@host:6379"
    )
    ...
    sentinel_password: Optional[str] = Field(
        None, description="password for the sentinel nodes"
    )
    ...
    def has_connection_target(self) -> bool:
        return any(
            value is not None
            for value in (self.host, self.url, self.startup_nodes, self.sentinel_nodes)
        )
