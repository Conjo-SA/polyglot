...
const AutoRotationView: React.FC<AutoRotationViewProps> = ({
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <Text className="font-semibold text-gray-900">{t("autoRotationView.title")}</Text>
          <Badge color={autoRotate ? "green" : "gray"} size="xs">
            {autoRotate ? t("autoRotationView.enabled") : t("autoRotationView.disabled")}
          </Badge>
          {autoRotate && rotationInterval && (
            <>
              <Text className="text-gray-400">•</Text>
              <Text className="text-sm text-gray-600">{t("autoRotationView.every", { interval: rotationInterval })}</Text>
            </>
          )}
        </div>
      </div>
        ...
        <div className="space-y-3">
          ...
          {lastRotationAt && (
            <div className="flex items-center gap-2 p-3 bg-gray-50 border border-gray-200 rounded-md">
              ...
              <div className="flex-1">
                <Text className="font-medium text-gray-700">{t("autoRotationView.lastRotation")}</Text>
          ...
          {(keyRotationAt || nextRotationAt) && (
            <div className="flex items-center gap-2 p-3 bg-gray-50 border border-gray-200 rounded-md">
              <ClockIcon className="w-4 h-4 text-gray-500" />
              <div className="flex-1">
                <Text className="font-medium text-gray-700">{t("autoRotationView.nextScheduledRotation")}</Text>
          ...
          {autoRotate && !lastRotationAt && !keyRotationAt && !nextRotationAt && (
            <div className="flex items-center gap-2 p-3 bg-gray-50 border border-gray-100 rounded-md">
              <ClockIcon className="w-4 h-4 text-gray-500" />
              <Text className="text-gray-600">{t("autoRotationView.noHistory")}</Text>
            </div>
          )}
        </div>
      )}
      ...
      {!autoRotate && !lastRotationAt && !keyRotationAt && !nextRotationAt && (
        <div className="flex items-center gap-2 p-3 bg-gray-50 border border-gray-100 rounded-md">
          ...
          <Text className="text-gray-600">{t("autoRotationView.notEnabled")}</Text>
        </div>
      )}
    </div>
  );
    ...
    return (
      <div className={`bg-white border border-gray-200 rounded-lg p-6 ${className}`}>
        <div className="flex items-center gap-2 mb-6">
          <div>
            <Text className="font-semibold text-gray-900">{t("autoRotationView.title")}</Text>
            <Text className="text-xs text-gray-500">{t("autoRotationView.cardSubtitle")}</Text>
          </div>
        </div>
        {content}
      </div>
  ...
  return (
    <div className={`${className}`}>
      <Text className="font-medium text-gray-900 mb-3">{t("autoRotationView.title")}</Text>
      {content}
    </div>
  );
};
