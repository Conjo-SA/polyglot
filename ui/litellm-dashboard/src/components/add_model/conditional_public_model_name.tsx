...
const ConditionalPublicModelName: React.FC = () => {
  ...
  const publicNameTooltipContent = (
    <>
      <div className="mb-2 font-normal">{t("addModel.publicModelNameTooltipDesc")}</div>
      <div className="mb-2 font-normal">
        <strong>{t("addModel.publicModelNameTooltipExampleLabel")}</strong> {t("addModel.publicModelNameTooltipExampleText")}{" "}
        <code className="bg-gray-700 px-1 py-0.5 rounded-sm text-xs">example-name</code>
        {t("addModel.publicModelNameTooltipExampleAnd")}{" "}
        <code className="bg-gray-700 px-1 py-0.5 rounded-sm text-xs">openai/qwen-plus-latest</code>{" "}
        {t("addModel.publicModelNameTooltipExampleEnd")}
      </div>
      <div className="mb-2 font-normal">
        <strong>{t("addModel.publicModelNameTooltipUsageLabel")}</strong> {t("addModel.publicModelNameTooltipUsageText")}{" "}
        <code className="bg-gray-700 px-1 py-0.5 rounded-sm text-xs">model = &quot;example-name&quot;</code>
      </div>
      <div className="font-normal">
        <strong>{t("addModel.publicModelNameTooltipResultLabel")}</strong> {t("addModel.publicModelNameTooltipResultTextPre")}{" "}
        <code className="bg-gray-700 px-1 py-0.5 rounded-sm text-xs">qwen-plus-latest</code>{" "}
        {t("addModel.publicModelNameTooltipResultTextPost")}
      </div>
    </>
  );

  const liteLLMModelTooltipContent = <div>{t("addModel.modelNameTooltip")}</div>;
  ...
  const columns = [
    ...
    },
    {
      title: (
        <span className="flex items-center">
          {t("addModel.liteLLMModelNameCol")}
  ...
  return (
    <>
      <Form.Item
        label={t("addModel.modelMappingsLabel")}
        name="model_mappings"
        ...
        rules={[
          {
            ...
            validator: async (_, value) => {
              if (!value || value.length === 0) {
                throw new Error(t("addModel.modelMappingsRequired"));
              }
              ...
              if (invalidMappings.length > 0) {
                throw new Error(t("addModel.modelMappingsInvalid"));
              }
            },
          },
        ]}
