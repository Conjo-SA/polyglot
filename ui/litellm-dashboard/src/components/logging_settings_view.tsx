...
export function LoggingSettingsView({
      ...
      <div className="space-y-3">
        ...
        {loggingConfigs.length > 0 ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="block text-xs text-blue-600">
                        {t("loggingSettingsView.parametersConfigured", { count: Object.keys(config.callback_vars).length })}
                      </span>
                    </div>
                  </div>
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 border border-gray-200">
            ...
            <span className="text-gray-500 text-sm">{t("loggingSettingsView.noLoggingIntegrations")}</span>
          </div>
        )}
      </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="block text-xs text-red-600">{t("loggingSettingsView.disabledForThisKey")}</span>
                    </div>
                  </div>
                  <Tag color="red">{t("loggingSettingsView.disabled")}</Tag>
                </div>
              );
            })}
          </div>
    ...
    return (
      <div className={`bg-white border border-gray-200 rounded-lg p-6 ${className}`}>
        <div className="flex items-center gap-2 mb-6">
          <div>
            <span className="block font-semibold text-gray-900">{t("loggingSettingsView.title")}</span>
            <span className="block text-xs text-gray-500">{t("loggingSettingsView.subtitle")}</span>
          </div>
        </div>
        {content}
      </div>
  ...
  return (
    <div className={`${className}`}>
      <span className="block font-medium text-gray-900 mb-3">{t("loggingSettingsView.title")}</span>
      {content}
    </div>
  );
}
