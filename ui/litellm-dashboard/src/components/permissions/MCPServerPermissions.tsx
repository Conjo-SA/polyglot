  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <Text className="font-semibold text-gray-900">{t("objectPermissionsView.mcpServers")}</Text>
        <Badge color={blocksAllMcpServers ? "red" : "blue"} size="xs">
          {blocksAllMcpServers ? t("objectPermissionsView.blocked") : grantsAllProxyMcpServers ? t("objectPermissionsView.all") : totalCount}
        </Badge>
      </div>
      ...
      {blocksAllMcpServers ? (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-red-50 border border-red-200">
          ...
          <Text className="text-red-700 text-sm">{t("objectPermissionsView.noMcpServersBlocked")}</Text>
        </div>
      ) : grantsAllProxyMcpServers ? (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-50 border border-blue-200">
          <ServerIcon className="h-4 w-4 text-blue-400" />
          <Text className="text-blue-700 text-sm">{t("objectPermissionsView.allProxyMcpServers")}</Text>
        </div>
      ) : totalCount > 0 ? (
              ...
              <div key={index} className="space-y-2">
                ...
                >
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    {item.type === "server" ? (
                      <Tooltip title={t("objectPermissionsView.fullId", { id: item.value })} placement="top">
                  ...
                  {hasToolRestrictions && (
                    <div className="flex items-center gap-1 shrink-0 whitespace-nowrap">
                      ...
                      <span className="text-xs text-gray-500">{toolsForServer.length === 1 ? t("objectPermissionsView.tool") : t("objectPermissionsView.tools")}</span>
                      {isExpanded ? (
                  ...
                  >
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      ...
                      <span className="ml-1 px-1.5 py-0.5 text-[9px] font-semibold text-purple-600 bg-purple-50 border border-purple-200 rounded-sm uppercase tracking-wide shrink-0">
                        {t("objectPermissionsView.toolset")}
                      </span>
                    </div>
                    {toolCount > 0 && (
                      <div className="flex items-center gap-1 shrink-0 whitespace-nowrap">
                        <span className="text-xs font-medium text-gray-600">{toolCount}</span>
                        <span className="text-xs text-gray-500">{toolCount === 1 ? t("objectPermissionsView.tool") : t("objectPermissionsView.tools")}</span>
                        {isExpanded ? (
                          <ChevronDownIcon className="h-3.5 w-3.5 text-gray-400 ml-0.5" />
                        ) : (
                          <ChevronRightIcon className="h-3.5 w-3.5 text-gray-400 ml-0.5" />
        ...
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 border border-gray-200">
          ...
          <Text className="text-gray-500 text-sm">{t("objectPermissionsView.noMcpConfigured")}</Text>
        </div>
      )}
    </div>
  );
