...
describe("valueFormatterSpend", () => {
  it("should return 'R$ 0' when the value is exactly zero", () => {
    expect(valueFormatterSpend(0)).toBe("R$ 0");
  });

  it("should format numbers >= 1,000,000 as BRL millions", () => {
    expect(valueFormatterSpend(1_000_000)).toBe("R$ 1M");
    expect(valueFormatterSpend(2_500_000)).toBe("R$ 2.5M");
    expect(valueFormatterSpend(10_000_000)).toBe("R$ 10M");
  });

  it("should format numbers >= 1,000 as BRL thousands", () => {
    expect(valueFormatterSpend(1_000)).toBe("R$ 1k");
    expect(valueFormatterSpend(5_500)).toBe("R$ 5.5k");
    expect(valueFormatterSpend(999_999)).toBe("R$ 999.999k");
  });

  it("should format numbers below 1,000 as plain BRL amounts", () => {
    expect(valueFormatterSpend(1)).toBe("R$ 1");
    expect(valueFormatterSpend(99.99)).toBe("R$ 99.99");
    expect(valueFormatterSpend(999)).toBe("R$ 999");
  });
