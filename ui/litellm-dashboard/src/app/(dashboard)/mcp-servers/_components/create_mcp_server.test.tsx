...
describe("CreateMCPServer", () => {
  ...
  describe("when HTTP transport is selected", () => {
    ...
    it("should warn that Polyglot auth is disabled when True Passthrough is selected", async () => {
      await selectHttpTransport();
      ...
      await waitFor(() => {
        expect(
          screen.getByText("True Passthrough disables Polyglot authentication for this server"),
        ).toBeInTheDocument();
      });
    });
    ...
    it.each([["True Passthrough (no Polyglot auth)"], ["OAuth Delegate (client-supplied upstream token)"]])(
    ...
    it.each([
      ["true_passthrough", "True Passthrough (no Polyglot auth)"],
