...
describe("dataUtils", () => {
  ...
  describe("getSpendString", () => {
    ...
    it("should format spend with BRL sign", () => {
      expect(getSpendString(1234.5, 2)).toBe("R$ 1.234,50");
      expect(getSpendString(-2500, 0)).toBe("R$ -2.500");
    });

    it("should return threshold string for very small values", () => {
      expect(getSpendString(0.0000004, 6)).toBe("< R$ 0,000001");
      expect(getSpendString(-0.0000004, 6)).toBe("< R$ 0,000001");
    });

    it("should respect custom decimals", () => {
      expect(getSpendString(0.01234, 3)).toBe("R$ 0,012");
      expect(getSpendString(999.9999, 1)).toBe("R$ 1.000,0");
    });
  });
